...
export function VisitorView({
    ...
    return [...allPlaces]
      .sort((a, b) => {
        const aDate = Math.max(
          ...[a?.uploadedAt || 0, ...a.reviews.map((r) => r.date || 0)]
        );
        const bDate = Math.max(
          ...[b?.uploadedAt || 0, ...b.reviews.map((r) => r.date || 0)]
        );
        return bDate - aDate;
      })
      .slice(0, 6);
  ...
  const getRecommendedPlaces = () => {
    return allPlaces
      .filter((p) => p.reviews.length > 0 && calculateAverageRating(p) >= 4)
      .slice(0, 6);
  };
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <div className="mb-12 text-center">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Explore local restaurants, hotels, cafes, mountains, and visitable
          places. Book your next adventure or visit.
        </p>
      </div>
      ...
      {(categoryFilter !== "all" || searchQuery !== "") && (
        <div className="mb-12">
          <SectionTitle
            ...
            subtitle={`Found ${filteredPlaces.length} place${
      ...
      {searchQuery === "" &&
        categoryFilter === "all" &&
        getLatestPlaces().length > 0 && (
          <div className="mb-12">
            <SectionTitle
              title="🆕 Latest Additions"
              subtitle="Recently added or reviewed"
            />
        ...
        getRecommendedPlaces().length > 0 && (
          <div className="mb-12">
            <SectionTitle
              ...
              subtitle="Highly rated places in the community"
            />
